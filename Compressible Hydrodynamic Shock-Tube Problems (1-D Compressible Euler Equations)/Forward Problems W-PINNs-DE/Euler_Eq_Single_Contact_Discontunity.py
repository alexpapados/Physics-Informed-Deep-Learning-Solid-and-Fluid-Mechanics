"""
Forward Problem for 1-D Euler Equations for Compressible Flow: Single Contact Discontinuity
...
def IC(x):
    ...
    u_init = 0.1*np.ones((x.shape[0]))                                             # u - initial condition
    p_init = np.ones((x.shape[0]))                                                 # p - initial condition
...
def main():
    ...
    epochs = 44350                                                        # Number of iterations
    ...
    t = np.linspace(0, 2, num_t)                                          # Partitioned time axis
    ...
    scipy.io.savemat('Single_Contact_Discontinuity.mat', {'x': x, 't': t,'rho': u_pred[:,0],
                                                              'u': u_pred[:,2],
                                                              'p': u_pred[:,1]})
