""" Inverse Problem for 1-D Euler Equations for Compressible Flow (Mao et al,2020) -- Total Energy Excluded
...
class PINNs():
    def __init__(self, X, x_i, x_l, x_r, rho_i, u_i, p_i, rho, u, p, layers):
        ...
        self.optimizer = torch.optim.LBFGS(
            ...
            tolerance_change = 7e-9,                     # Termination tolerance on function value/parameter changes
            history_size = 1000
        )
    ...
    def loss_bc(self,xl,tl,xr,tr):
        ...
        # Loss of BCs
        loss_bcs = ((rho_l - 1.4) ** 2).mean() + ((rho_r - 1.0) ** 2).mean() + \
                ((u_l - 0.1) ** 2).mean() + ((u_r - 0.1) ** 2).mean() + \
                ((p_l - 1.0) ** 2).mean() + ((p_r - 1.0) ** 2).mean()
        return loss_bcs
    ...
    def loss_func(self):
        ...
        if BC == 'Yes':
            ...
            loss = 0.01 * (((rho_pred - self.rho) ** 2).mean() + ((u_pred - self.u) ** 2).mean() + \
                           ((p_pred - self.p) ** 2).mean() + f_pred + 0.01 * loss_ics )
        ...
        print(
                'Iteration %d, Loss: %e,  gamma_exact: 1.4, gamma_PINNs: %.5f,' %
                (
                    self.iter,
                    loss.item(),
                    self.gamma.item()
...
# Initialization
layers = [2, 60, 60, 60,60, 3]                                              # [Input, Neurons per Layer, Output]
...
num_ic_train = 1000                                                         # Sampling from IC
num_b_train = 100                                                           # Sampling from Boundary
num_f_train = 2000                                                          # Sampling from Interior


Exact_rho = np.real(data['rho_exact'])                                      # Exact density
Exact_u = np.real(data['u_exact'])                                          # Exact velocity
Exact_p = np.real(data['p_exact'])                                          # Exact pressure
...
id_ic = np.random.choice(num_x, num_ic_train, replace=False)                # Randomly chosen points for IC
...
x_ic = x_grid[0,id_ic][:, None]                                              # x of IC
t_ic = t_grid[0,id_ic][:, None]                                              # t of IC
