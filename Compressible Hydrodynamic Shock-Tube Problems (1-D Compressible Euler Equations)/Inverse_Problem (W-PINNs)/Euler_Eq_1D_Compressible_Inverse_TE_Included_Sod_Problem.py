""" Inverse Problem for 1-D Euler Equations for Compressible Flow: The Sod Problem -- Total Energy Included
...
class PINNs():
    def __init__(self, X, E,rho,u,p, layers):
        ...
        self.optimizer = torch.optim.LBFGS(
            ...
            tolerance_grad=1e-9,                    # Default termination tolerance on first order optimality
            tolerance_change=1e-9,                  # Default termination tolerance on function value/parameter changes
            history_size=1000
        )
    ...
    def loss_func(self):
        y_pred = self.net_y(self.x, self.t)
...
Exact_rho = np.real(data['r_full']).T                                      # Exact density
Exact_u = np.real(data['u_full']).T                                        # Exact velocity
Exact_p = np.real(data['p_full']).T                                        # Exact pressure
Exact_E = np.real(data['E_full']).T                                        # Exact total energy
